...
2. **Audit target file** — identify color mode, mode switch, shorthand vars, hardcoded colors, colored sections, JS hardcoded colors
3. **Inject `:root` block** — add theme token block at top of `<style>` (see token template in supporting-info); use `Edit`, never rewrite whole file
4. **Harmonize** — fix what variable injection can't reach (see harmonize steps in supporting-info)
5. **Report** using the report format in supporting-info
...
3. **Colors** - analyze visually for all 14 slots
...
6. **Background** - inspect canvas carefully before assuming solid
7. **Effects** - analyze visually
...
- ≥ 4.5:1 text/bg contrast; `primaryLight` same hue as `primary`; dark themes bg < 20% lightness, light > 90%
- Use dominant hues with sharp accents - avoid timid even distributions
- Build cohesion around 1-2 anchor hues; accents must feel deliberately chosen
- **AVOID pure `#000000` and `#ffffff`** unless the theme's identity demands it

### Typography

Never use: Inter, Roboto, Arial, system-ui, Helvetica, or Space Grotesk.
...
terminal=0-2px, brutalist=0px, corporate=4-6px, editorial=6-8px, SaaS=8-12px, art deco=2-4px, playful=16-24px

### Background

Prioritize depth over solid colors. Prefer gradient meshes, noise textures, geometric patterns, or layered transparencies.
...
- Use preset-template.html as structural base, but **design freely** to express the theme's full potential
...
Read `~/.claude/skills/theme-factory/preset-template.html` for the full template structure. Replace every `<placeholder>` with the actual extracted value. `<script id="theme-data">` must be valid JSON. Every CSS variable in `:root` must have a concrete value. The `<body>` must contain only the generic component showcase.

The template opens with `<!DOCTYPE html><html lang="en" data-theme="<name>">`.

For Path C themes: the template is a **base reference only** - deviate freely in layout and component design. The JSON data block must still be valid and complete.

---

## Apply Mode: Token Template
